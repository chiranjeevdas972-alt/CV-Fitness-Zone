...
export function Dashboard() {
  ...
  useEffect(() => {
    ...
    const unsubscribeMembers = onSnapshot(membersQuery, 
      (snapshot) => {
        ...
        const active = members.filter(m => m.paymentStatus === 'paid').length;
        setStats(prev => ({
          ...prev,
    ...
    const unsubscribeAnnouncements = onSnapshot(announcementsQuery, 
      (snapshot) => {
        setLatestAnnouncements(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })));
      },
  ...
  const statCards = [
    { name: 'Total Members', value: stats.totalMembers, icon: Users, color: 'bg-blue-500', trend: '+12%' },
    { name: 'Active Members', value: stats.activeMembers, icon: UserCheck, color: 'bg-green-500', trend: '+5%' },
    { name: 'Monthly Revenue', value: `₹${stats.monthlyRevenue}`, icon: CreditCard, color: 'bg-red-500', trend: '+18%' },
    { name: 'Daily Check-ins', value: stats.dailyCheckins, icon: Clock, color: 'bg-orange-500', trend: '+24%' },
  ];

  const quickActions = [
    { name: 'Add Member', icon: Plus, path: '/members', color: 'bg-red-600' },
    { name: 'Mark Attendance', icon: CalendarCheck, path: '/attendance', color: 'bg-zinc-800' },
    { name: 'Record Payment', icon: CreditCard, path: '/payments', color: 'bg-zinc-800' },
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-zinc-500 mt-1">Here's what's happening at CV Fitness Zone today.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {statCards.map((stat) => (
          <div key={stat.name} className="bg-white dark:bg-zinc-900 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm hover:shadow-md transition-shadow group">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-zinc-900 p-8 rounded-3xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
          <div className="flex items-center justify-between mb-8">
            ...
            <select className="bg-zinc-100 dark:bg-zinc-800 border-none rounded-xl text-[10px] font-black uppercase tracking-widest px-4 py-2 focus:ring-2 focus:ring-red-600">
